...
app.get("/api/health", (req, res) => {
  res.json({
    status: "online",
    system: "PROJECT ARCHON // COGNITIVE FORENSICS LAB CORE",
...
app.get("/api/spectrogram-data", (req, res) => {
  res.json({
    sampleRate: 48000,
...
app.get("/api/memory-dump", (req, res) => {
  res.json({
...
// 5. Terminal Execution Engine (Volatile Forensics & GDB commands)
...
app.post("/api/neuro-core/chat", async (req, res) => {
...
app.post("/api/verify-shard", (req, res) => {
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[+] Cognitive Forensics Lab Server running on http://0.0.0.0:${PORT}`);
  });
}
